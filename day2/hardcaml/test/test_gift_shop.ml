(* test for gift_shop module - hardware computes triangular sums for arithmetic sequences.
   testbench precomputes intersection parameters which require division/modulo. *)
...
(* compute pattern parameters: step, start, end for a pattern of given dimensions *)
...
(* compute intersection of pattern sequence with input range.
   returns (lower, upper, n) where lower is first valid id, upper is last,
   and n is (upper-lower)/step. this requires division which stays in testbench. *)
...
let run_testbench patterns ranges (sim : Harness.Sim.t) =
  ...
  (* stream all pattern/range combinations to hardware *)
